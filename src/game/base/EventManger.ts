 ...
 * @Last Modified time: 2020-07-15 20:53:48
 */
class EventManger extends BaseClass {
    ...
    public add(target: egret.DisplayObject, thisobj, config: EventObj, allowSimultaneous: boolean = false) {
        ...
        // target.addEventListener(egret.TouchEvent.TOUCH_CANCEL, this.onEnd, this)
    ...
    private onTapRecognized(e: GestureEvent): void {
        ...
        if (event.tapMode == TapMode.Scale) {
            event.config.onTapRecognized(e, event.beginXY)
        }
    }
    ...
    private onBegin(e: egret.TouchEvent) {
        ...
        if (event.touchCount == 1) {
            event.beginXY[0] = e.stageX
            event.beginXY[1] = e.stageY
        }
        if (event.touchCount == 2) {
            event.beginXY[2] = e.stageX
            event.beginXY[3] = e.stageY
        }
    }

    private onMove(e: egret.TouchEvent) {
        let event = this.getEvent(e.currentTarget)
        if (!event.hasBegin || event.tapMode == TapMode.Scale) {
            return
        }
...
class EventObj {
    ...
    onTap: Function
    onEnd: Function
    constructor(thisobj, onTapRecognized: Function, onMove: Function, onTap: Function, onEnd: Function) {
        this.onTapRecognized = onTapRecognized.bind(thisobj)
        this.onMove = onMove.bind(thisobj)
        this.onTap = onTap.bind(thisobj)
        this.onEnd = onEnd.bind(thisobj)
    }
