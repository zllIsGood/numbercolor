...
class Main extends eui.UILayer {
	...
	public static dataVersion = "10100"
	// 平台相关配置
	public static version: string = "11000"; //AB 10900
	...
	public static adId: string = '503'; //趣趣简笔画5  国学博物馆502  神奇的画展503
	// 关卡配置
	...
	private loadConfig2() {
		return new Promise((resolve, reject) => {
			// RES.getResAsync('paintConfig_json', (data) => {
			RES.getResByUrl(App.ins().getResRoot() + 'resource/config/paintConfig.json', (data) => {
				GlobalConfig.initPaint(data)
				resolve()
			}, this)
		})
	}
	...
	public static initVersion() {
		let s = CacheUtil.get(Constant.DATA_VERSION)
		if (s && s != '') {
			StoryUtil.isFinish = true
		}
		if (s != Main.dataVersion) {
			CacheUtil.set(Constant.DATA_VERSION, Main.dataVersion)
		}
	}
}
window["Main"] = Main