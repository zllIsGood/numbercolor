...
class MovieClip extends egret.MovieClip {
	...
	/**playCount -1 循环   isLast 到最后一帧 */
	public playFile(name: string,
		playCount: number = 1,
		compFun: () => void = null,
		remove: boolean = true, isLast = false): void {
		...
		let dataPathS = this.name.lastIndexOf("/");
		let dataPath = this.name.substring(dataPathS + 1)
		this.jsonData = RES.getRes(`${dataPath}_json`);
		this.texture = RES.getRes(`${dataPath}_png`);
		...
		if (this.jsonData && this.texture) {
			...
			if (RES.hasRes(`${dataPath}_json`)) {
				RES.getResAsync(`${dataPath}_json`, (data) => {
					this.jsonData = data;
					this.createBody();
				}, this)

				RES.getResAsync(`${dataPath}_png`, (data) => {
					this.texture = data;
					if (this.stage) {
						MovieClip.addDisplayObject(this, this.texture.bitmapData);
					}
